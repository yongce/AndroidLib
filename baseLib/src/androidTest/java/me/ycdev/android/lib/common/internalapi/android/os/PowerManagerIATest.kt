...
class PowerManagerIATest {
    ...
    @Test
    fun test_shutdown() {
        assumeAvailable(PowerManagerIA.shutdownAvailability())
        assertTrue("failed to reflect #shutdown", PowerManagerIA.checkReflectShutdown())
    }

    @Test
    fun test_crash() {
        assumeAvailable(PowerManagerIA.crashAvailability())
        assertTrue("failed to reflect #crash", PowerManagerIA.checkReflectCrash())
    }

    @Test
    fun test_goToSleep() {
        assumeAvailable(PowerManagerIA.goToSleepAvailability())
